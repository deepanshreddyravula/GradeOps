...
def get_exam_submissions(exam_id: str, current_user: dict = Depends(get_current_instructor)):
    return {
        "submissions": list_submissions_by_exam(exam_id,current_user["id"]),
        "evaluations": list_evaluations_by_exam(exam_id,current_user["id"])
    }

@router.get("/student/{student_id}")
def get_student_submissions(student_id: str, current_user: dict = Depends(get_current_instructor)):
    return {
        "submissions": list_submissions_by_student(student_id,current_user["id"]),
        "evaluations": list_evaluations_by_student(student_id,current_user["id"])
    }